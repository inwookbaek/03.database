/*
	...
	1. PL/SQL의 장점
	   ...
	   1) block구조로 다수의 SQL문을 한번에 Oracle DB서버로 전송해서 처리하기 때문에
		 ...
		 4) variable(변수), constant(상수), cursor(커서), exception(예외처리)등을 정의할 수
		    있고 SQL문장과 procedural문장에서 사용할 수 있다.
	...
	3. PL/SQL의 종류
	   ...
	   1) anonymous block(익명블럭) : 이름이 없는 블럭으로 보통 1회성으로 실행되는 블럭이다.
		 2) strored procedure : 매개변수를 전달 받을 수 있고 재사용이 가능하며 보통은 연속실행거나
		    구현이 복잡한 트랜잭션을 수행하는 PL/SQl블럭으로 "데이베이스 서버 안에 저장"이 된다.
		 ...
		 3) function : procedure와 유사하지만 다른 점을 처리결과를 호출한 곳으로 반환해 주는 값이
		    있다는 것이다. 다만 in 파라미터만 사용할 수 있고, 반드시 반환될 값의 데이터타입을 return문
				안에 선언해야 한다. 또한, PL/SQl블럭내에서 return문을 사용하여 반드시 값을 리턴해야 한다.
		 4) package : 패키지는 오라클 데이터베이스 서버에 저장되어 있는 procedure와 fuction의 집합이다.
		    패키지는 선언부와 본문 두 부분으로 나누어 관리한다.
		 5) trigger : insert, delete, update등이 특정 테이블에서 실행될 때 자동으로 수행하도록 정의한
		    프로시저아다. 트리거는 테이블과 별오로 database에 저장(객ㅊ)된다. 트리거는 table에 대해서만
			 정의할 수 있다.
...
-- 2. if ~ then ~ else ~ end if
-- hr.employees에서 commission이 있으면 보너스를 지급, 없드면 지급하지 않음
...
/*
  ...
  create or replace procedure 프로시저명(arg1 in 데이터타입, ... argn in 데이타입) is
	begin
	end;
*/
