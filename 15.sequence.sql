/*
	...
	1. 유일한 값을 생성해 주는 오라클객체중 하다.
	2. 시퀀스를 생성하면 키본키와 같이 순차적으로 증가하는 컬럼을 자동으로 생성이 가능하다.
	...
	1. create sequence 시퀀스명
		 ...
		 [maxvalue n | nomaxvalue] 	-- 생략하면 기본값 nomaxvalue(9999999999999)
		 [minvalue n | nominvalue]  -- 생략하면 기본값 nominvalue
		 [cycle | nocycle]          -- 생략하면 기본값 nocycle
		 [cache | nocache]          -- 생략하면 기본값 nocache
		 
		 ... start with : 시퀀스의 시작랍을 정의 n을 1000이라 지정하면 1000부터 시작 정의하지 않으면 1부터시작
		 ... increment by : 자동증가값을 정의, n 10이면 10씩증가 
